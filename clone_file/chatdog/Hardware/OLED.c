...
/**
  ...
  * @param  Start 汉字串在字库中的起始位置序列
  * @param  End   汉字串在字库中的结束位置序列
  * @retval 无
  */
void OLED_ShowChineseString(uint8_t Line, uint8_t Column, uint8_t Start, uint8_t End)
{
	uint8_t i,j=0;
	uint16_t addr=32*Start;
	for(j=Start;j<(End+1);j++)
	{
		OLED_SetCursor((Line-1)*2,(Column-1+j-Start)*16);
		for(i=0;i<16;i++)
		{
			OLED_WriteData(OLED_Font[addr]);
			addr+=1;
		}
		OLED_SetCursor((Line-1)*2+1,(Column-1+j-Start)*16);
		for(i=0;i<16;i++)
		{
			OLED_WriteData(OLED_Font[addr]);
			addr+=1;
...
void OLED_ShowPicture(uint8_t Line, uint8_t Column, uint8_t x, uint8_t y,uint8_t Start)
{
	...
	for(i=Line;i<(y/8)+Line;i++)
	{
		...
		for(j=0;j<x;j++)
		{
			OLED_WriteData(OLED_Picture[addr]);
			addr+=1;
		}
	}
}
...
/**
  * @brief   显示GIF动图(单帧图片像素只能为（128×64）)
  * @param   Num 图片的帧数
  * @retval  无
  */
